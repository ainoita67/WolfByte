...
$router->protected('PUT', '/incidencias/{id}', 'Controllers\\IncidenciaController@update', [10, 20, 30, 40]); // Se enviaran los datos de una incidencia para modificarla
...
$router->protected('GET', '/user/{id}',          'Controllers\\UsuarioController@show', [10, 20, 30, 40]); // Se reciben los datos del usuario con el id que se mande
...
$router->protected('GET', '/reservas-necesidades/{id_reserva}/necesidades',         'Controllers\\NecesidadReservaController@index', [10, 20, 30, 40]);
...
// EDIFICIOS
$router->protected('GET', '/edificios', 'Controllers\\EdificioController@index', [10, 20, 30, 40]); //seleccionar todos los edificios
...
$router->protected('GET', '/caracteristicas', 'Controllers\\CaracteristicaController@index', [10, 20, 30, 40]);
...
// Espacios 
// RESERVAS
$router->protected('GET', '/mis-reservas/{id_usuario}','Controllers\\ReservaController@misReservas', [10, 20, 30, 40]); // Devuelve las reservas del usuario autenticado
...
$router->protected('GET', '/reservaMaterial/material/{id}','Controllers\\ReservaMaterialController@showMaterial', [20, 30, 40]); // Devuelve las reservas de un carrito específico por ID de carrito


//Espacios

$router->protected('GET', '/espacios', 'Controllers\\EspacioController@index', [10, 20, 30, 40]);
$router->protected('GET', '/espacios/activos', 'Controllers\\EspacioController@indexActivos', [10, 20, 30, 40]);
$router->protected('GET', '/espacios/{id}', 'Controllers\\EspacioController@show', [10, 20, 30, 40]);
...
$router->protected('GET', '/aulas', 'Controllers\\EspacioController@indexAulas', [10, 20, 30, 40]); // Devuelve solo los espacios que son aulas organizadas por edificio y planta
$router->protected('POST', '/aulas/disponibles', 'Controllers\\EspacioController@indexAulasDisponibles', [10, 20, 30, 40]); // Devuelve solo los espacios que son aulas organizadas por edificio y planta y que estén disponibles en el rango de fecha y hora especificado
$router->protected('GET', '/otrosespacios', 'Controllers\\EspacioController@indexOtrosEspacios', [10, 20, 30, 40]); // Devuelve solo los espacios que no son aulas ni salon de actos organizadas por edificio y planta
$router->protected('POST', '/otrosespacios/disponibles', 'Controllers\\EspacioController@indexOtrosEspaciosDisponibles', [10, 20, 30, 40]); // Devuelve solo los espacios que no son aulas ni salon de actos organizadas por edificio y planta y que estén disponibles en el rango de fecha y hora especificado
$router->protected('POST', '/portatiles/disponibles', 'Controllers\\MaterialController@indexCarritos', [20, 30, 40]); // Devuelve solo los espacios que son salon de actos organizados por edificio y planta

// RESERVAS ESPACIOS
$router->protected('GET', '/reservaEspacio','Controllers\\ReservaEspacioController@index', [10, 20, 30, 40]); // Devuelve todas las reservas de tipo “espacio”
$router->protected('GET', '/mis-reservas-espacio','Controllers\\ReservaEspacioController@misReservas', [10, 20, 30, 40]); // Devuelve todas las reservas de espacio de un usuario autenticado
$router->protected('POST', '/reservaEspacio','Controllers\\ReservaEspacioController@store', [10, 20, 30, 40]); // Añade una nueva reserva de un espacio
...
$router->protected('GET', '/reservaEspacio/espacio/{id}','Controllers\\ReservaEspacioController@showEspacio', [10, 20, 30, 40]); // Devuelve las reservas de un espacio específico por ID de espacio
$router->protected('PUT', '/reservaEspacio/{id}','Controllers\\ReservaEspacioController@update', [10, 20, 30, 40]); // Cambia los datos de una reserva de espacio (comprobar disponibilidad)
...
$router->protected('GET', '/reservas_permanentes', 'Controllers\\ReservaPermanenteController@index', [10, 20, 30, 40]); //consultar todas las reservas permanentes activas
...
$router->protected('GET', '/reservas_permanentes/recurso/{id_recurso}', 'Controllers\\ReservaPermanenteController@showActivasRecurso', [10, 20, 30, 40]); //consultar todas las reservas permanentes activas de un recurso
...
$router->protected('GET', '/portatiles/reservas',                   'Controllers\\PortatilController@reservas', [20, 30, 40]); // Listar todas las reservas
$router->protected('GET', '/portatiles/reservas/usuario/{id_usuario}', 'Controllers\\PortatilController@reservasByUsuario', [20, 30, 40]); // Reservas por usuario
$router->protected('GET', '/portatiles/reservas/{id}',              'Controllers\\PortatilController@reserva', [20, 30, 40]); // Ver reserva por ID
$router->protected('POST', '/portatiles/reservas',                  'Controllers\\PortatilController@createReserva', [20, 30, 40]); // Crear reserva
$router->protected('POST', '/portatiles/reservas/disponibilidad',   'Controllers\\PortatilController@disponibilidad', [20, 30, 40]); // Verificar disponibilidad
$router->protected('PUT', '/portatiles/reservas/{id}',              'Controllers\\PortatilController@updateReserva', [20, 30, 40]); // Actualizar reserva
$router->protected('PATCH', '/portatiles/reservas/{id}',            'Controllers\\PortatilController@patchReserva', [20, 30, 40]); // Actualizar parcialmente
$router->protected('PATCH', '/portatiles/reservas/{id}/unidades',   'Controllers\\PortatilController@patchUnidades', [20, 30, 40]); // Actualizar unidades
$router->protected('DELETE', '/portatiles/reservas/{id}',           'Controllers\\PortatilController@deleteReserva', [20, 30, 40]); // Eliminar reserva
// RECURSO
$router->protected('GET', '/recurso', 'Controllers\\RecursoController@index', [10, 20, 30, 40]); //Nos devuelve id y descripción de todos los recursos que estén en la base de datos
$router->protected('GET', '/recurso/activos', 'Controllers\\RecursoController@indexActivos', [10, 20, 30, 40]); //Nos devuelve id y descripción del recurso por id
...
// LIBERACIÓN PUNTUAL
$router->protected('GET', '/liberaciones', 'Controllers\\LiberacionPuntualController@index', [10, 20, 30, 40]); //Consultar todas las liberaciones puntuales
$router->protected('GET', '/liberaciones/recurso/{id_recurso}', 'Controllers\\LiberacionPuntualController@showByRecurso', [10, 20, 30, 40]); //Consultar todas las liberaciones puntuales de un recurso
...
$router->protected('POST', '/liberaciones', 'Controllers\\LiberacionPuntualController@store', [10, 20, 30, 40]); //Añadir una liberación puntual
