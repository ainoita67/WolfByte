...
class EspacioService
{
    ...
    public function createEspacio(array $input): array
    {
        ...
        $data['es_aula'] = $data['es_aula'] ?? 0;
        $data['activo'] = $data['activo'] ?? 1;
        $data['especial'] = $data['especial'] ?? 0;
    ...
    public function updateEspacio(array $input): array
    {
        if(!$this->getEspacioById($input['id_recurso'])){
            throw new \Exception("Espacio no encontrado", 404);
        }
        // Validar datos requeridos - CORREGIDO: es_aula NO es required
        $data = Validator::validate($input, [
        ...
        try {            
            ...
            $result=$this->getEspacioById($data['id_recurso']);
        ...
        if (!$result) {
            // Obtener más información del error
            error_log("Resultado falso - Revisar logs de PDO");
            throw new \Exception("No se pudo editar el espacio. Verifique los datos e intente nuevamente.", 500);
        }

        // Obtener el espacio creado para devolverlo completo
        return [
            'status' => 'success',
            'data' => $this->getEspacioById($data['id_recurso'])
        ];
    }
